...
source "proxmox-iso" "ubuntu-noble" {
  template_name            = "ubuntu-noble-cloud-init-enabled-base-image"
...
build {
  ...
  provisioner "shell" {
    inline = ["echo 'packer' | sudo -S rm  /etc/cloud/cloud-init.disabled"]
  }

  provisioner "shell" {
    inline = ["echo 'packer' | sudo -S cloud-init clean"]
  }
}